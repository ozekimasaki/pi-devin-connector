...
8. **Always-fresh auto-refresh**: pi only calls `refreshModels` at startup / post-login / manual refresh. To keep the picker current for the whole session, `extensions/index.ts` additionally triggers `modelRegistry.refresh({ providers: ['devin'] })` on `session_start` (new/resume/fork), on `agent_settled`, and on a 15-min `unref`'d interval (cleared on `session_shutdown`). All paths go through `autoRefreshDevinCatalog`, which early-outs when there's no credential or `isCatalogStale` says the cached catalog is inside its 10-min TTL — so the network is only touched when the data is actually stale. Errors are swallowed (stale catalog + working chat beats a crashed session).
