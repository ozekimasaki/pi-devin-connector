 ...
 * events so the caller can stream straight into pi's event stream.
 *
 ...
 *   - All models the user's api_key is entitled to
 *   - Streaming (uses Connect-streaming envelope, emits deltas as they arrive)
 *   - pi `SimpleStreamOptions` hooks: fetch override, extra headers,
 *     timeout, onPayload / onResponse
 *
...
import { getCachedUserJwt, type FetchImpl } from './auth.js';
 ...
 * Single-process scope is enough: pi lives in one runtime for a TUI
 ...
 * cascade_id per pi conversation is fine; reuse across turns to
 ...
 *   #6  tool_calls: repeated ChatToolCall       (assistant turns)
 *   #7  tool_call_id: string                    (tool-result turns)
...
// ----------------------------------------------------------------------------
// Response parsing — pull deltas out of each frame
// ----------------------------------------------------------------------------
 ...
 *     #3  delta_text (string) — VISIBLE answer text
 *     #5  finish_reason (varint — 10 = "tool_calls" observed)
 *     #6  ToolCallDelta {
 ...
 *     #9  reasoning/thinking delta (string)
 *     #12 (fixed64) some_hash
 *     #17 (string) message_uuid
 *     #28 UsageStats { #1 label, ... }
 *   }
 *
 * #9 appears both at top-level (thinking delta) AND inside #7 (model_name).
...
function* decodeChatFrame(proto: Buffer): Generator<CloudChatEvent> {
  for (const f of iterFields(proto)) {
    if (f.num === 3 && f.wire === 2 && Buffer.isBuffer(f.value)) {
      ...
      // consumers render it in a collapsed thinking block instead of
      ...
      // exa.codeium_common_pb.StopReason → finish reason.
...
/** Flattened response view handed to the `onResponse` hook. */
export interface CloudChatResponseInfo {
  status: number;
...
export interface CloudChatRequest {
   ...
   * may emit `tool_call_start`/`_args` events instead of plain text.
   */
  tools?: ToolDef[];
  ...
  signal?: AbortSignal;
  /**
   * Fetch implementation override (pi's `SimpleStreamOptions.fetch`).
   * Defaults to `globalThis.fetch`.
   */
  fetch?: FetchImpl;
  /**
   * Extra HTTP headers merged over the Connect-RPC defaults
   * (pi's `SimpleStreamOptions.headers`). A `null` value suppresses a
   * default header of the same name.
   */
  headers?: Record<string, string | null | undefined>;
  /**
   * Request timeout in milliseconds, applied to the time-to-first-byte
   * window (pi's `SimpleStreamOptions.timeoutMs`). Stream idleness after
   * first byte stays governed by {@link CLOUD_STREAM_IDLE_MS}.
   */
  timeoutMs?: number;
  /**
   * Called with the fully-built request body (Connect-streaming envelope)
   * before it is sent (pi's `SimpleStreamOptions.onPayload`). Return a
   * Buffer/Uint8Array to replace the payload; return undefined to keep it.
   */
  onPayload?: (
    payload: Buffer,
  ) => Buffer | Uint8Array | undefined | Promise<Buffer | Uint8Array | undefined>;
  /**
   * Called after the HTTP response headers arrive and before the body
   * stream is consumed (pi's `SimpleStreamOptions.onResponse`).
   */
  onResponse?: (response: CloudChatResponseInfo) => void | Promise<void>;
}
...
export async function* streamChatEvents(req: CloudChatRequest): AsyncGenerator<CloudChatEvent> {
  ...
  const userJwt = await getCachedUserJwt(req.apiKey, host, req.signal, fetchImpl);
  ...
  const catalog = await getCachedCatalog(req.apiKey, host, req.signal, fetchImpl).catch(() => null);
  if (catalog) {
  ...
  let body: Buffer = frameConnectStream(proto, true);

  // pi `onPayload` hook — inspect or replace the outgoing request body.
  // A returned Buffer/Uint8Array replaces the payload; anything else keeps it.
  if (req.onPayload) {
    const replaced = await req.onPayload(body);
    if (replaced !== undefined && replaced !== null) {
      body = replaced instanceof Buffer ? replaced : Buffer.from(replaced);
    }
  }

  // Compose caller signal with a TTFB timeout. If the cloud takes longer
  // than the timeout to start the response, abort. Once any byte
  // arrives we cancel the TTFB timer and start the per-chunk idle timer
  // inside the read loop instead.
  const ttfbMs = req.timeoutMs ?? CLOUD_STREAM_TTFB_MS;
  const ttfbController = new AbortController();
  const ttfbTimer = setTimeout(() => ttfbController.abort(new Error(`cloud-direct: time-to-first-byte timeout (${ttfbMs}ms)`)), ttfbMs);
  const ttfbSignal = ttfbController.signal;
  const initialSignal: AbortSignal = req.signal ? AbortSignal.any([req.signal, ttfbSignal]) : ttfbSignal;

  // Merge caller headers over the Connect-RPC defaults; a `null` value
  // suppresses the default header of the same name.
  ...
  if (req.headers) {
    for (const [k, v] of Object.entries(req.headers)) {
      if (v === null || v === undefined) delete requestHeaders[k];
  ...
  let resp: Response;
  try {
    resp = await fetchImpl(`${host}/exa.api_server_pb.ApiServerService/GetChatMessage`, {
      method: 'POST',
  ...
  // pi `onResponse` hook — invoked after headers arrive, before the body
  // stream is consumed.
  if (req.onResponse) {
    await req.onResponse({ status: resp.status, headers: headersToRecord(resp.headers) });
  }
  ...
  try {
    ...
    // Always clear the idle timer, and cancel the underlying body stream on
    // any non-clean exit so the TCP connection is released.
    if (idleTimer) clearTimeout(idleTimer);
