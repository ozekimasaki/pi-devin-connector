 ...
 *   indistinguishable from a transient server fault.
 *
...
async function fetchCatalog(
  ...
  const userJwt = await getCachedUserJwt(apiKey, host, signal, fetchImpl);
  ...
  let resp: Response;
  try {
    resp = await fetchImpl(`${host}/exa.api_server_pb.ApiServerService/GetCascadeModelConfigs`, {
      method: 'POST',
 ...
 * good enough for pi's single-account-at-a-time usage pattern.
 *
...
export async function getCachedCatalog(
  ...
  const promise = fetchCatalog(apiKey, host, signal, fetchImpl);
  inFlight = promise;
  inFlightKey = key;
  try {
